...
export class LoginComponent implements OnInit {
  ...
  submit() {
    ...
    if(index == -1) {
      let emailValid = this.users.findIndex(item => item.email == this.loginForm.value.email  )
      let passValid = this.users.findIndex(item => item.password == this.loginForm.value.password  )

      if(emailValid == -1){
        this.toaster.error("الايميل غير صحيح" , "" , {
        ...
        this.toaster.error("كلمه المرور غير صحيحه" , "" , {
          disableTimeOut: false,
          titleClass: "toastr_title",
          messageClass: "toastr_message",
          timeOut:5000,
      ...
      this.service.login(model).subscribe(res => {
        ...
        setTimeout(() => this.router.navigate(['/subjects']) , 100)
      })
    }
